...
app.post('/detach-ue', (req, res) => {
    ...
    const deleteDeployment = `minikube kubectl -- delete deployment ${prefixedUECode}-deployment`;
...
app.post('/traffic-ue', (req, res) => {
    ...
    // Static values for NF type and pod name
    const nfType = 'ueransim-ue001';  // Adjust this as necessary
    const nfPod = 'ueransim-ue001-pod-name';  // Replace with the actual pod name you want to target

    // Command to send traffic
    const pingCommand = `minikube kubectl -- exec ${nfPod} -- ping google.com -I uesimtun0 -c 5`;

    conn.on('ready', () => {
        console.log('SSH Connection established');

        // Execute the ping command
        conn.exec(pingCommand, (err, stream) => {
            if (err) {
            ...
            stream.on('data', (data) => {
                const output = data.toString();
                combinedOutput += output; // Append the output
                console.log(`Ping Command Output: ${output}`);
            });

            stream.stderr.on('data', (data) => {
                const errorOutput = data.toString();
                combinedOutput += errorOutput; // Append error output
                console.error(`Ping Command Error: ${errorOutput}`);
            });

            stream.on('close', (code) => {
                console.log(`Ping command stream closed with code: ${code}`);
                conn.end(); // Close the connection after all commands are done

                if (code === 0) {
                    res.json({
                        success: true,
                        message: `Traffic sent successfully to ${nfPod}.`,
                        output: combinedOutput
                    });
                } else {
                    console.error('Failed to send traffic, exit code:', code);
                    res.status(500).json({ message: 'Failed to send traffic', output: combinedOutput });
                }
            });
        });
    }).connect({
...
app.post('/attach-ue', (req, res) => {
    ...
    conn.on('ready', () => {
        ...
        console.log(`Executing command: ${createConfigMap}`);
        conn.exec(changeDir + createConfigMap, (err, stream) => {
            if (err) {
                console.error('Error executing createConfigMap:', err);
                return res.status(500).json({ message: `SSH Command Error: ${err.message}` });
            }
            ...
            stream.on('data', (data) => {
                ...
                console.log(`ConfigMap Creation Output: ${output}`);
            });
            ...
            stream.stderr.on('data', (data) => {
                ...
                console.error(`ConfigMap Creation Error: ${errorOutput}`);
            });

            stream.on('close', (code) => {
                console.log(`createConfigMap stream closed with code: ${code}`);
                conn.end();

                // Always return combined output regardless of success or failure
                res.json({ message: `ConfigMap creation attempted for UE ${ueCode}.`, output: combinedOutput });
            });
        });
    }).connect({
        host: vmConfig.host,
